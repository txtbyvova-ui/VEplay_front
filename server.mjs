...
function createClientFolders(folderId) {
  for (const cat of CATEGORIES) fs.mkdirSync(path.join(folderPath(folderId), cat), { recursive: true })
}
...
async function handleRequest(req, res) {
  ...
  if (pathname.startsWith('/admin/')) {
    ...
    if (pathname === '/admin/clients' && req.method === 'POST') {
      ...
      try {
        createClientFolders(folderId)
      } catch (e) {
        return json(res, { error: `Не удалось создать папку: ${e.message}` }, 500)
      }
      const newUser = {
        username, name, role: 'user',
        categories: [...CATEGORIES], folderId,
        ...hashPassword(password),
      }
      users.push(newUser)
      await saveUsers(users)
      return json(res, { folderId, name, username, password }, 201)
    }
    ...
    if (tl && req.method === 'GET') {
      ...
      const out = {}
      for (const cat of CATEGORIES) out[cat] = scanFolder(folderId, cat)
      return json(res, out)
    }
